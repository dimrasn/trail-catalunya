...
export function buildPrompt(filteredRaces, filters) {
  ...
  const filterLine = phrases.length
    ? `My filters: ${phrases.join('; ')}.`
    : 'No filters applied yet — feel free to ask me what I want.'
  ...
  const truncationNote = total > MAX_INLINE
    ? `\n(Showing the first ${MAX_INLINE} of ${total} matches — narrow the filters on the site for a tighter list.)`
    : ''
...
Here are the matching races (drive times are from Plaça Glòries, Barcelona — not your location):
