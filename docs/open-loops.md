...
| L16 | **MCP expected-month parity — BUILT, not deployed.** `grouping.ts` + `filters_core.ts` now carry expectedMonth with mirrored tests (`041457c`). Awaiting re-audit, then a code-only MCP deploy. | agent | after re-audit | open |
| L17 | **Verify the live MCP version before deploying.** Merged `AGENTS.md` is internally inconsistent — one line says multi-value MCP is DEPLOYED & VERIFIED, another says main is ahead of live at v11. Confirm the actual live version (call `initialize`) before any deploy so we know what we are shipping on top of. | agent | before MCP deploy | open |
