...
| 2026-08-31 | First real crawl reading (logged manually by an agent — the weekly cron fires but records nothing; see note). 30d: googlebot 230/226 race pages, oai-searchbot 225/226, applebot 109, chatgpt-user 139 hits/29 pages, perplexity/gptbot/bing light. **Plumbing ALIVE** — Google + OpenAI have ~full /race/ coverage and real ChatGPT users are landing. BUT the AI-citation probe is still unrun → crawl success, NOT GEO success. And claudebot: 213 hits, only **4** /race/ pages — it reads homepage/for-agents, never descends. | (1) **Cron doesn't record** — a Claude cron git-committing markdown from a shared worktree (usually on another branch) fails silently; lastRunAt updates. Fix: write readings to a Supabase table server-side, not git [owner decision: band-aid allow-rule vs structural table — pending]. (2) **claudebot /race/ discoverability** — check internal linking / sitemap from for-agents+homepage into /race/. (3) **Run the AI-citation probe** — the only real GEO signal, still unrun. | 2026-09-14 | _open_ |
| 2026-08-31 | Strategy (outside review, Codex): "enrichment-as-GEO-primary-source" rejected as sunk-cost — normalizing public organizer data is aggregation, not primary-source origin; Google says structured data isn't a special requirement for AI answers. Crawl ≠ citation ≠ demand. | Do NOT commit a cycle to enrichment-for-GEO. Next cycle = distribution + demand validation on what's already shipped (the ask-box is LIVE): dogfood with **cold external runners**, cheap plumbing (MCP registry, Bing/IndexNow), close this loop, THEN 3–5 hand-curated answer pages as an experiment — no new fields until real use names one. See ROADMAP.md strategic correction 2026-08-31. | 2026-09-21 | _open_ |
