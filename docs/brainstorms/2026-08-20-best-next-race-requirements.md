...
- **Sourcing the taste/description layer** — RESOLVED in principle (content-growth review, 2026-08-20), still deferred as a build:
  ...
  - Attribute model (Catalan-trail-organizer review, 2026-08-20 — corrects the naive list):
    - **The frame runners filter on FIRST is distance + vertical gain (D+) — which we already have as structured data.** The soft "taste" attributes are tiebreakers *inside* a distance/D+ bracket, not the foundation. So the cheapest, highest-signal first win needs no scraping at all: compute the **km-esforç difficulty index** (`km + D+/100`, the native FEEC/FEDME metric) + mean gradient from data we already hold, and derive **altitude exposure** and **season/heat-vs-cold** from date+geography.
    - **Extract first (highest decision-weight × reliably stated):** difficulty index (derived), cutoffs/`talls horaris` (in the reglament), competitive/championship status (Copa Catalana FEEC / Campionat — separates points-chasers from day-outers), max altitude, aid-station density + `material obligatori` (self-sufficiency).
    - **Catalonia-specific gates an outsider misses:** FEEC **circuit membership**, and the **federation-license requirement** (FEEC license vs. day-license/insurance) — a real friction gate; the reglament/`inscripcions` PDF is where the honest high-value data hides, not the glossy homepage.
    - **Second wave, low-confidence by design:** technicality (footing) + climb runnability + atmosphere — the best discriminators but the ones organizers systematically undersell, so they must default to **"unknown — no evidence"** loudly rather than inherit the site's optimistic adjectives ("espectacular").
    - **Drop** `scenic` as low/med/high (every organizer claims "high" → no discriminating signal); replace with factual **setting tags** (alpine · forest · ridgeline · coastal · vineyard · volcanic · national park · monastery/historic). Keep start-time-of-day, tradition (from edition count "XXV edició"), food/family (botifarrada, kids' cursa) as real but lower-weight flavour.
    - The rendered description is still composed FROM these attributes through the **runner's lens**, never a summary of the scraped page.
