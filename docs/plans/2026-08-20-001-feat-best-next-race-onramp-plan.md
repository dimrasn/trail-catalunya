...
- R4. **On-site intent logging** — **SUPERSEDED by `docs/plans/2026-08-25-001-feat-intent-logging-plan.md`** (Dima 2026-08-25: log raw goal text by default, credential-free with the RPC as the trust boundary). The derived-category-default wording below is no longer authoritative; see that plan's Supersession section.  ~~one row per submission capturing the intent chips, the current filters, and a DERIVED goal category (mapped from any free text)... unlinked, user-volunteered content... purged on a fixed retention.~~
...
- **KTD3 — Intent log ingest — SUPERSEDED by `docs/plans/2026-08-25-001-feat-intent-logging-plan.md`.** That plan's Supersession section is authoritative: Dima chose **credential-free** (the `anon` `SECURITY DEFINER` RPC is the trust boundary and does full semantic validation + an atomic write-volume ceiling), NOT a route-handler-held credential. The credentialed-route wording below is retained only as the documented **upgrade path** if the directional signal ever needs to be decision-grade. (Trackability T1's `/api/out` still shares whatever ingest contract that plan settles.)  ~~A Next.js route handler holds the write credential server-side... rate-limited + body-capped.~~
