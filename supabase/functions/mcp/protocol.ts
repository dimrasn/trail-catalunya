...
export const INSTRUCTIONS = [
  ...
  'fits", not a dump. Rank on four axes, all from fields already in the response (no fetching):',
  '  • LOW-FAFF: drive_minutes_from_barcelona — lead with it; nearer wins ties.',
  '  • ENJOYMENT / CHARACTER: taste_summary + taste_flags (night, technicality) + taste.editorial',
  '    from get_race — match the user\'s stated vibe (scenic, runnable, night, family). Filter',
  '    taste_flags PERMISSIVELY (absent = unknown, not "no").',
  ...
  '  • NOVELTY / PB: surface something new, or (only if a training connector is present) readiness +',
  '    a rough projected finish per the COMPOSES-WITH-TRAINING-DATA rules below.',
  'Every axis except the last is ZERO-SETUP — never require a connector. Always keep the honesty',
  'rule: registration status + start time are not in this data — tell the user to verify at the url.',
  '',
