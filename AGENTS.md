...
**Taste layer Slice 1 — site LIVE, MCP pending a CLI deploy (2026-08-22).** The
taste layer (editorial + clean character attributes, honesty-labelled by
claim_strength; operational facts deferred to Slice 2) is generated by
`scripts/build-taste.mjs` → `docs/enrichment/2026-batch/parsed/taste.json`
(git-versioned source), gated by the pure module `app/lib/taste.js` ↔
`supabase/functions/mcp/taste_view.ts` (parity-tested). **Site: LIVE** — `races.js`
imports the JSON at build; 80/84 profiles join; the race page shows an "Our take"
+ "Character" section. **MCP: code committed but NOT deployed** — `tools.ts`
imports `mcp/taste.json` (bundled) + per-tool projection (list=taste_available
+summary, get_race=full). **Operational trap:** the Supabase MCP
`deploy_edge_function` tool is inline-only, and `mcp/taste.json` is ~165KB — too
large to hand-inline reliably (a JSON syntax error would 500 the public MCP). So
the MCP taste deploy must go through the **Supabase CLI from disk** (reads the
files, no inlining): `supabase login` → `supabase link --project-ref
qaebfhbdfjvzhmvcjroz` → `supabase functions deploy mcp --no-verify-jwt`. The CLI
(v2.90.0) is installed but not linked and there's no `SUPABASE_ACCESS_TOKEN`, so
an agent cannot do this unattended — it needs Dima's login. Until then the live
MCP is v10 (no taste); the site is ahead.
