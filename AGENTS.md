...
**Multi-select site filters + multi-value MCP filters — BUILT, NOT DEPLOYED
...
a drive-time band; legacy scalar inputs remain supported. Prepared on
`feat/multi-select-filters-clean` as three filter-only commits on top of current
`main`, with the corrected `dimrasn@gmail.com` author identity. Site deployment
requires merging/pushing `main`; MCP deployment remains a separate Supabase CLI
step from the repo root (see deploy trap below).
